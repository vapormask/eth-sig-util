...
const bob = { 
  vaporyPrivateKey: '7e5374ec2ef0d91761a6e72fdf8f6ac665519bfdf6da0a2329cf0d804514b816',
...
test("Getting bob's encryptionPublicKey", async t => {
  ...
  const result = await sigUtil.getEncryptionPublicKey(bob.vaporyPrivateKey)
...
test("Bob decryptSafely message that Alice encryptSafely for him", async t => {
  ...
  const plaintext = sigUtil.decryptSafely(result, bob.vaporyPrivateKey);
...
test("Bob decrypts message that Alice sent to him", t => {
  ...
  const result = sigUtil.decrypt(encryptedData, bob.vaporyPrivateKey);
...
test('Decryption failed because version is wrong or missing', t =>{
  ...
  t.throws( function() { sigUtil.decrypt(badVersionData, bob.vaporyPrivateKey)}, 'Encryption type/version not supported.')
});
...
test('Decryption failed because nonce is wrong or missing', t => {
  ...
  t.throws(function() { sigUtil.decrypt(badNonceData, bob.vaporyPrivateKey)}, 'Decryption failed.')
...
test('Decryption failed because ephemPublicKey is wrong or missing', t => {
  ...
  t.throws(function() { sigUtil.decrypt(badEphemData, bob.vaporyPrivateKey)}, 'Decryption failed.')
});
...
test("Decryption fails because you are not the recipient", t => {
  ...
  t.throws(function() { sigUtil.decrypt(encryptedData, alice.vaporyPrivateKey)}, 'Decryption failed.')
});

test('signedTypeData', (t) => {
  t.plan(8)
  const utils = sigUtil.TypedDataUtils
  const privateKey = vapUtil.sha3('cow')
  const address = vapUtil.privateToAddress(privateKey)
  const sig = sigUtil.signTypedData(privateKey, { data: typedData })

  t.equal(utils.encodeType('Mail', typedData.types),
    'Mail(Person from,Person to,string contents)Person(string name,address wallet)')
  t.equal(vapUtil.bufferToHex(utils.hashType('Mail', typedData.types)),
    '0xa0cedeb2dc280ba39b857546d74f5549c3a1d7bdc2dd96bf881f76108e23dac2')
  t.equal(vapUtil.bufferToHex(utils.encodeData(typedData.primaryType, typedData.message, typedData.types)),
  ...
  t.equal(vapUtil.bufferToHex(utils.hashStruct(typedData.primaryType, typedData.message, typedData.types)),
    '0xc52c0ee5d84264471806290a3f2c4cecfc5490626bf912d01f240d7a274b371e')
  t.equal(vapUtil.bufferToHex(utils.hashStruct('EIP712Domain', typedData.domain, typedData.types)),
    '0xf2cee375fa42b42143804025fc449deafd50cc031ca257e0b194a650a912090f')
  t.equal(vapUtil.bufferToHex(utils.sign(typedData)),
    '0xbe609aee343fb3c4b28e1df9e632fca64fcfaede20f02e86244efddf30957bd2')
  t.equal(vapUtil.bufferToHex(address), '0xcd2a3d9f938e13cd947ec05abc7fe734df8dd826')
